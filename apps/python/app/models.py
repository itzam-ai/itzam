...
class Users(Base):
    ...
    provider_key: Mapped[List['ProviderKey']] = relationship('ProviderKey', back_populates='user')
...
class Knowledge(Base):
    ...
    workflow: Mapped[List['Workflow']] = relationship('Workflow', back_populates='knowledge')
    resource: Mapped[List['Resource']] = relationship('Resource', back_populates='knowledge')


class ModelSettings(Base):
    __tablename__ = 'model_settings'
    __table_args__ = (
        PrimaryKeyConstraint('id', name='model_settings_pkey'),
    )

    id: Mapped[str] = mapped_column(String(256), primary_key=True)
    temperature: Mapped[decimal.Decimal] = mapped_column(Numeric(3, 2))
    temperature_preset: Mapped[str] = mapped_column(Enum('STRICT', 'BALANCED', 'CREATIVE', 'CUSTOM', name='temperature_preset'))
    max_tokens: Mapped[int] = mapped_column(Integer)
    max_tokens_preset: Mapped[str] = mapped_column(Enum('SHORT', 'MEDIUM', 'LONG', 'CUSTOM', name='max_tokens_preset'))
    created_at: Mapped[datetime.datetime] = mapped_column(DateTime(True), server_default=text('CURRENT_TIMESTAMP'))
    updated_at: Mapped[datetime.datetime] = mapped_column(DateTime(True))

    workflow: Mapped[List['Workflow']] = relationship('Workflow', back_populates='model_settings')
...
class Provider(Base):
    ...
    provider_key: Mapped[List['ProviderKey']] = relationship('ProviderKey', back_populates='provider')


class ApiKey(Base):
    __tablename__ = 'api_key'
    __table_args__ = (
        ForeignKeyConstraint(['user_id'], ['auth.users.id'], name='api_key_user_id_users_id_fk'),
...
class Model(Base):
    __tablename__ = 'model'
    __table_args__ = (
        ForeignKeyConstraint(['provider_id'], ['provider.id'], name='model_provider_id_provider_id_fk'),
    ...
    has_reasoning_capability: Mapped[bool] = mapped_column(Boolean, server_default=text('false'))
    ...
    max_tokens: Mapped[int] = mapped_column(Integer)
    input_per_million_token_cost: Mapped[Optional[decimal.Decimal]] = mapped_column(Numeric(10, 6))
    output_per_million_token_cost: Mapped[Optional[decimal.Decimal]] = mapped_column(Numeric(10, 6))
    provider_id: Mapped[Optional[str]] = mapped_column(String(256))

    provider: Mapped[Optional['Provider']] = relationship('Provider', back_populates='model')
    chat: Mapped[List['Chat']] = relationship('Chat', back_populates='last_model')
    workflow: Mapped[List['Workflow']] = relationship('Workflow', back_populates='model')
    chat_message: Mapped[List['ChatMessage']] = relationship('ChatMessage', back_populates='model')
...
class ProviderKey(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['provider_id'], ['provider.id'], name='provider_key_provider_id_provider_id_fk'),
        ForeignKeyConstraint(['user_id'], ['auth.users.id'], name='provider_key_user_id_users_id_fk'),
    ...
    provider: Mapped[Optional['Provider']] = relationship('Provider', back_populates='provider_key')
...
class Chat(Base):
    __tablename__ = 'chat'
    __table_args__ = (
        ForeignKeyConstraint(['last_model_id'], ['model.id'], name='chat_last_model_id_model_id_fk'),
        ForeignKeyConstraint(['user_id'], ['auth.users.id'], name='chat_user_id_users_id_fk'),
...
class ChatMessage(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['chat_id'], ['chat.id'], name='chat_message_chat_id_chat_id_fk'),
        ForeignKeyConstraint(['model_id'], ['model.id'], name='chat_message_model_id_model_id_fk'),
        PrimaryKeyConstraint('id', name='chat_message_pkey'),
        Index('chat_message_created_at_idx', 'created_at')
    )

    id: Mapped[str] = mapped_column(String(256), primary_key=True)
    role: Mapped[str] = mapped_column(Enum('user', 'assistant', 'system', 'data', name='chat_message_role'))
    content: Mapped[str] = mapped_column(Text)
    cost: Mapped[decimal.Decimal] = mapped_column(Numeric(10, 6), server_default=text("'0'::numeric"))
    ...
    model: Mapped[Optional['Model']] = relationship('Model', back_populates='chat_message')
    message_file: Mapped[List['MessageFile']] = relationship('MessageFile', back_populates='message')


class Context(Base):
    __tablename__ = 'context'
    __table_args__ = (
        ForeignKeyConstraint(['workflow_id'], ['workflow.id'], name='context_workflow_id_workflow_id_fk'),
    ...
    resource: Mapped[List['Resource']] = relationship('Resource', back_populates='context')
    thread_context: Mapped[List['ThreadContext']] = relationship('ThreadContext', back_populates='context')
...
class Thread(Base):
    ...
    thread_context: Mapped[List['ThreadContext']] = relationship('ThreadContext', back_populates='thread')
...
class MessageFile(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['message_id'], ['chat_message.id'], name='message_file_message_id_chat_message_id_fk'),
...
class Resource(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['context_id'], ['context.id'], name='resource_context_id_context_id_fk'),
        ForeignKeyConstraint(['knowledge_id'], ['knowledge.id'], name='resource_knowledge_id_knowledge_id_fk'),
        PrimaryKeyConstraint('id', name='resource_pkey'),
        Index('resource_context_id_idx', 'context_id'),
        Index('resource_knowledge_id_idx', 'knowledge_id')
    )

    id: Mapped[str] = mapped_column(String(256), primary_key=True)
    active: Mapped[bool] = mapped_column(Boolean, server_default=text('true'))
    status: Mapped[str] = mapped_column(Enum('PENDING', 'PROCESSED', 'FAILED', name='resource_status'), server_default=text("'PENDING'::resource_status"))
    url: Mapped[str] = mapped_column(String(1024))
...
class Run(Base):
    __tablename__ = 'run'
    __table_args__ = (
        ForeignKeyConstraint(['model_id'], ['model.id'], name='run_model_id_model_id_fk'),
        ForeignKeyConstraint(['thread_id'], ['thread.id'], name='run_thread_id_thread_id_fk'),
        ForeignKeyConstraint(['workflow_id'], ['workflow.id'], name='run_workflow_id_workflow_id_fk'),
    ...
    id: Mapped[str] = mapped_column(String(256), primary_key=True)
    origin: Mapped[str] = mapped_column(Enum('SDK', 'WEB', 'API', name='run_origin'))
    status: Mapped[str] = mapped_column(Enum('RUNNING', 'COMPLETED', 'FAILED', name='run_status'))
...
class ThreadContext(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['context_id'], ['context.id'], name='thread_context_context_id_context_id_fk'),
        ForeignKeyConstraint(['thread_id'], ['thread.id'], name='thread_context_thread_id_thread_id_fk'),
        PrimaryKeyConstraint('id', name='thread_context_pkey')
    )
    ...
    context: Mapped['Context'] = relationship('Context', back_populates='thread_context')
    thread: Mapped['Thread'] = relationship('Thread', back_populates='thread_context')
...
class Attachment(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['run_id'], ['run.id'], name='attachment_run_id_run_id_fk'),
...
class Chunks(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['resource_id'], ['resource.id'], name='chunks_resource_id_resource_id_fk'),
        ForeignKeyConstraint(['workflow_id'], ['workflow.id'], name='chunks_workflow_id_workflow_id_fk'),
...
class RunResource(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['resource_id'], ['resource.id'], name='run_resource_resource_id_resource_id_fk'),
        ForeignKeyConstraint(['run_id'], ['run.id'], name='run_resource_run_id_run_id_fk'),
