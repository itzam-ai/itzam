...
export type UseTextStreamOptions = {
  ...
  delay?: number; // Initial delay in milliseconds before starting to stream
};
...
function useTextStream({
  ...
  }, [textStream, reset, processStringTypewriter, processAsyncIterable, delay]);
...
export type ResponseStreamProps = {
  ...
  delay?: number; // Initial delay in milliseconds before starting to stream
  onComplete?: () => void;
  onTypingChange?: (isTyping: boolean) => void; // Callback for typing state changes
...
function ResponseStream({
  ...
  }, [delay, onTypingChange]);

  useEffect(() => {
    if (isComplete) {
      onTypingChange?.(false);
    }
  }, [isComplete, onTypingChange]);
